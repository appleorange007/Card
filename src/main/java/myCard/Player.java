...
class Player {
    ...
    Player(SHinterface sh, Image cardspic, Graphics g, Hand hand, Image back, Image backSW,
            Image burntPic, Image[] pointer, Score score)
    {
        this.sh = sh;
    ...
    private boolean isValidCard(Card card, String command)
    {
        ...
        if (card.getValue() == 2) {// 2 can be played at anytime
            ...
            return true;
        } else if (card.getValue() == 10 || fourOfAKind(card) == true) {// pile is burn and its
                                                                        // players turn again
            burnt = true;
            for (int n = 0; n < 51; n++)
                pile[n] = null;
    ...
    class WaitforMsg implements Runnable {
        ...
        public void run()
        {
            do {
                ...
                {
                    // sh.addMsg("Msg: " + otherplayermsg);//------------------------------------msg
                    // from server
            ...
            if (cardno.equals("pickup")) {// other players picks up pile
                ...
                if (cardno2.equals("pickup")) {
                    ...
                    sh.addMsg(othernames[playernumber] + " played a "
                            + Card.getCardStringValue(numPlayed) + " and had to picked up the pile");
                } else {
                    try {
                        Card card = new Card(Integer.parseInt(cardno2), cardspic, sh, g);
                        for (int n = 51; n > 0; n--)
        ...
        private void otherdetails(String otherplayermsg, int commandlength)
        {
            ...
            for (int n = 0; n < 3; n++)
            {
                ...
                try {
                    for (int i = 0; i < 3; i++) {
                        Card card = new Card(Integer.parseInt(variables[4 * n + 1 + i]), cardspic,
                                sh, g);
                        faceup[n][i] = card;
                    }
                } catch (NumberFormatException b) {
            ...
            if (swap == true && swapdone == false) {// if performing card swap
                ...
                sendCommand("swap:" + inhand[0].getNumber() + ":" + inhand[1].getNumber() + ":"
                        + inhand[2].getNumber() + ":" + ontable[0].getNumber() + ":"
