...
public class Card
{
    ...
    private void getCardDetails(Image cardspic, Graphics tempg)
    {
        ...
        if (cardNumber < 14)
            ...
            cardSuit = 3;
        else if (cardNumber >= 40 && cardNumber < 53)
            cardSuit = 4;

        // Finding Value
        if (cardSuit == 1)
            cardValue = cardNumber + 1;
        else if (cardSuit == 2)
            cardValue = cardNumber - 12;
        else if (cardSuit == 3)
            cardValue = cardNumber - 25;
        else if (cardSuit == 4)
            cardValue = cardNumber - 38;

        // seperating card image from other card images
        tempg.drawImage(cardspic, -(cardValue - 2) * cardWidth, -(cardSuit - 1) * cardHeight, sh);
